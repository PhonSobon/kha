...
const content = {
  EN: {
    ...
    address: "No. 82ឈ, Dirt Road, Group 2, Borey 100 Khnang Village, Teuk Thla Commune, Russey Keo District, Phnom Penh",
    about: "About",
    leader: "KHA Leader",
    contact: "Contact",
  },
  KH: {
    ...
    address: "អាសយដ្ឋាន៖ អាគារ ៨២ឈ ផ្លូវលំ ក្រុមទី២ ភូមិបូរី១០០ខ្នង សង្កាត់ទឹកថ្លា ខណ្ឌឬស្សីកែវ រាជធានីភ្នំពេញ",
    about: "អំពីយើង",
    leader: "ប្រធានសមាគម",
    contact: "ទំនាក់ទំនង",
  }
...
export default function KHAFooter({ lang = "EN", setLang }) {
      ...
      <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-4 gap-8 items-center">
        ...
        <div className="flex flex-col items-start">
          <div className="flex items-center gap-3 mb-2">
            <Image src="/logo.png" alt="KHA Logo" width={40} height={40} className="rounded-3xl shadow bg-white" />
            <span className="font-bold text-lg">{t.name}</span>
          </div>
        </div>
        {/* Address */}
        <div>
          <span className="font-bold">{lang === "EN" ? "Address" : "អាសយដ្ឋាន"}</span>
        ...
        {/* Links */}
        <div>
          <span className="font-bold">{lang === "EN" ? "Links" : "តំណភ្ជាប់"}</span>
          <ul className="mt-2 space-y-1 text-sm">
            <li>
              <Link href="/about" className="hover:underline">{t.about}</Link>
            </li>
            <li>
              <Link href="/leader" className="hover:underline">{t.leader}</Link>
            </li>
          </ul>
        </div>
        {/* Contact */}
